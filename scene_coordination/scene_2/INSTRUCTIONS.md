# Scene 2: Standing Up For Others
...
## DEPENDENCY WARNING
This scene shares the level blueprint `/Game/Maps/Game/Main/Levels/SLs/SL_Main_Logic` with Scene 1.
**Before starting:** Check `../scene_1/STATUS.json`.
- If Scene 1 is "in_progress", STOP. Tell the user: "Scene 1 is currently being wired on the same level blueprint. I must wait."
- If Scene 1 is "complete" or "not_started", proceed.
- When checking out the level blueprint via `p4 edit`, verify no other agent has it locked.

## Notes
Step 6 is automated (paper release). Shares SL_Main_Logic level blueprint with Scene 1. MUST coordinate with Scene 1 agent -- same Blueprint file.

## Interaction Definitions

| Step | Event Name | Trigger | Actor | Description |
|------|-----------|---------|-------|-------------|
| 4 | `Scene2_KitchenTableNav` | Nav | TBD | NAV to kitchen table -> Heather skips to kitchen (LS_2_1) |
| 5 | `Scene2_IllustrationGrabbed` | Grip | TBD | GRIP illustration paper -> vignette plays (LS_2_2R) |
...
4. Remove `LOCK.json`.
5. Run `p4 submit -d "Scene 2: wired 2 interactions"` to submit the changelist.
