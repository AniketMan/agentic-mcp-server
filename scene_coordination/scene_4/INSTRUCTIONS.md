# Scene 4: Stepping Into Adulthood
...
## DEPENDENCY WARNING
This scene shares the level blueprint `/Game/Maps/Game/Trailer/Levels/SLs/SL_Trailer_Logic` with Scene 3.
**Before starting:** Check `../scene_3/STATUS.json`.
- If Scene 3 is "in_progress", STOP. Tell the user: "Scene 3 is currently being wired on the same level blueprint. I must wait."
- If Scene 3 is "complete" or "not_started", proceed.
- When checking out the level blueprint via `p4 edit`, verify no other agent has it locked.

## Notes
Shares SL_Trailer_Logic level blueprint with Scene 3. MUST coordinate -- same Blueprint file.

## Interaction Definitions

| Step | Event Name | Trigger | Actor | Description |
|------|-----------|---------|-------|-------------|
| 11 | `Scene4_PhoneGrabbed` | Grip | TBD | GRIP phone -> text sequence begins (LS_4_1) |
| 12 | `Scene4_TextAdvance1` | Trigger | TBD | TRIGGER advance text 1 (LS_4_2) |
| 13 | `Scene4_TextAdvance2` | Trigger | TBD | TRIGGER advance text 2 (LS_4_3) |
| 14 | `Scene4_DoorHandleGrabbed` | Grip | TBD | GRIP front door -> opens to restaurant (LS_4_4) |
...
4. Remove `LOCK.json`.
5. Run `p4 submit -d "Scene 4: wired 4 interactions"` to submit the changelist.
