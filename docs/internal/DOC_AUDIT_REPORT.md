...
| **C5** | **`GROUND_TRUTH.md` claims "There is no script generation"** | This document [2] makes a definitive statement that is factually untrue for the repository in its current state. Both `core/script_generator.py` and `scripts/project_inject_interactions.py` [18] exist and are tracked in git. |
...
- **G6: Incorrect Project Mechanics Mapping:** The `PROJECT_MECHANICS_CATALOG.md` [7] contains an incorrect mapping of scenes to level files, contradicting the `project_inject_interactions.py` script [18].
...
5.  **Correct Factual Errors:**
    -   Update the `PROJECT_MECHANICS_CATALOG.md` [7] to match the logic in the `project_inject_interactions.py` script [18].
...
[7] `PROJECT_MECHANICS_CATALOG.md`
...
[18] `scripts/project_inject_interactions.py`

---

## 10. Addendum: Engine Validation Results (Mar 05, 2026)

Since the initial audit, the binary editing pipeline was tested against the actual UE 5.6 engine (Oculus fork) via Perforce submission. The results are definitive.

### Tests Performed

| Changelist | Description | Result |
| :--- | :--- | :--- |
| 34278 | Extras-only edit: wired BeginPlay -> PrintString | Opened in editor. Wire visible. PrintString fired on Play. |
| 34279 | Tagged property removal (EnabledState) + SSEO patch | Opened in editor. BeginPlay fired automatically without manual enable. |

### UAssetAPI Bug: ScriptSerializationEndOffset (SSEO)

A critical bug was discovered in UAssetAPI: it recalculates `SerialSize` on save but does NOT recalculate `ScriptSerializationEndOffset` (SSEO). When tagged properties change size, the engine asserts with:

> `ScriptSerializationEndOffset X does not match offset during deserialization Y for object Z`

The fix is implemented in `core/post_save_patcher.py`, which performs a binary fixup pass after every UAssetAPI save. For K2Node exports:
...
### Export Table Binary Layout (Verified)

The export table entry is 112 bytes per entry in UE 5.6 (Oculus fork). Key fields verified by binary inspection and confirmed by Epic's Developer Assistant:
...
The original audit recommended adopting Workflow A. This addendum upgrades that from a recommendation to a **confirmed fact**: Workflow A works in production. The pipeline can wire nodes, enable/disable events, and modify tagged properties — all headlessly, all validated by the engine.

### New References

[19] `epic_sseo_confirmation.md`
[20] `core/post_save_patcher.py`
[21] `inject_fun_level.py`
[22] `inject_fun_level_v2.py`
