...
// Validation and safety handlers for ManusMCP.
...
FString FManusMCPServer::HandleValidateBlueprint(const FString& Body)
{
...
// ============================================================
// HandleSnapshotGraph
// POST /api/snapshot-graph { "blueprint": "...", "description": "Before adding teleport logic" }
// ============================================================

FString FManusMCPServer::HandleSnapshotGraph(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body"));
	...
	UE_LOG(LogTemp, Display, TEXT("ManusMCP: Snapshot '%s' created for '%s' (%d nodes, %d connections)"),
...
FString FManusMCPServer::HandleRestoreGraph(const FString& Body)
{
	TSharedPtr<FJsonObject> Json = ParseBodyJson(Body);
	if (!Json.IsValid()) return MakeErrorJson(TEXT("Invalid JSON body"));
	...
	FManusGraphSnapshot* Snapshot = Snapshots.Find(SnapshotId);
	if (!Snapshot)
	{
	...
	for (UEdGraph* Graph : AllGraphs)
	{
		...
		FManusGraphSnapshotData* SnapshotData = Snapshot->Graphs.Find(Graph->GetName());
	...
	for (const auto& Pair : Snapshot->Graphs)
	{
		...
		for (const FManusNodeRecord& NodeRec : Pair.Value.Nodes)
		{
