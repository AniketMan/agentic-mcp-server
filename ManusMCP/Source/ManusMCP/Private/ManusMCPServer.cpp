// ManusMCPServer.cpp
// Core HTTP server implementation for ManusMCP.
...
FString FManusMCPServer::JsonToString(TSharedRef<FJsonObject> JsonObj)
{
	FString Output;
...
TSharedPtr<FJsonObject> FManusMCPServer::ParseBodyJson(const FString& Body)
{
...
FString FManusMCPServer::MakeErrorJson(const FString& Message)
{
...
FString FManusMCPServer::UrlDecode(const FString& EncodedString)
{
	FString Result;
...
FAssetData* FManusMCPServer::FindBlueprintAsset(const FString& NameOrPath)
{
	// Exact match first
...
FAssetData* FManusMCPServer::FindMapAsset(const FString& NameOrPath)
{
	// Exact match first
...
UBlueprint* FManusMCPServer::LoadBlueprintByName(const FString& NameOrPath, FString& OutError)
{
	...
	if (MapAsset)
	{
		...
		if (World && World->PersistentLevel)
		{
			...
			if (LevelBP)
			{
				UE_LOG(LogTemp, Display, TEXT("ManusMCP: Loaded level blueprint from map '%s'"),
					*NameOrPath);
				return LevelBP;
			}
		}
...
UEdGraphNode* FManusMCPServer::FindNodeByGuid(
...
UClass* FManusMCPServer::FindClassByName(const FString& ClassName)
{
	// Direct lookup
...
bool FManusMCPServer::ResolveTypeFromString(
...
bool FManusMCPServer::SaveBlueprintPackage(UBlueprint* BP)
{
	if (!BP) return false;
	...
	if (CompileResult != 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("ManusMCP: Blueprint compilation crashed (SEH caught) for '%s'"),
			*BP->GetName());
	...
	if (SaveSEH != 0)
	{
		UE_LOG(LogTemp, Error, TEXT("ManusMCP: Package save crashed (SEH caught) for '%s'"),
			*BP->GetName());
		return false;
	}
...
TSharedRef<FJsonObject> FManusMCPServer::SerializeBlueprint(UBlueprint* BP)
{
...
TSharedPtr<FJsonObject> FManusMCPServer::SerializeGraph(UEdGraph* Graph)
{
...
TSharedPtr<FJsonObject> FManusMCPServer::SerializeNode(UEdGraphNode* Node)
{
...
TSharedPtr<FJsonObject> FManusMCPServer::SerializePin(UEdGraphPin* Pin)
{
...
FString FManusMCPServer::GenerateSnapshotId(const FString& BlueprintName)
{
	return FString::Printf(TEXT("%s_%s"),
		*BlueprintName, *FGuid::NewGuid().ToString(EGuidFormats::Short));
}

FManusGraphSnapshotData FManusMCPServer::CaptureGraphSnapshot(UEdGraph* Graph)
{
	FManusGraphSnapshotData Data;
		...
		for (UEdGraphPin* Pin : Node->Pins)
		{
			if (Pin && Pin->Direction == EGPD_Output)
			{
				for (UEdGraphPin* Linked : Pin->LinkedTo)
				{
					if (Linked && Linked->GetOwningNode())
					{
						FManusPinConnectionRecord Conn;
...
void FManusMCPServer::PruneOldSnapshots()
{
...
FString FManusMCPServer::HandleRescan()
{
	...
	UE_LOG(LogTemp, Display, TEXT("ManusMCP: Rescanned - %d Blueprints, %d Maps"),
...
void FManusMCPServer::RegisterHandlers()
{
...
bool FManusMCPServer::Start(int32 InPort, bool bEditorMode)
{
	Port = InPort;
	bIsEditor = bEditorMode;

	// ---- Scan asset registry ----
	UE_LOG(LogTemp, Display, TEXT("ManusMCP: Scanning asset registry..."));
	FAssetRegistryModule& ARM = FModuleManager::LoadModuleChecked<FAssetRegistryModule>("AssetRegistry");
	ARM.Get().SearchAllAssets(true);

	ARM.Get().GetAssetsByClass(UBlueprint::StaticClass()->GetClassPathName(), AllBlueprintAssets, true);
	UE_LOG(LogTemp, Display, TEXT("ManusMCP: Found %d Blueprint assets."), AllBlueprintAssets.Num());

	ARM.Get().GetAssetsByClass(UWorld::StaticClass()->GetClassPathName(), AllMapAssets, false);
	UE_LOG(LogTemp, Display, TEXT("ManusMCP: Found %d Map assets."), AllMapAssets.Num());
	...
	if (!Router.IsValid())
	{
		UE_LOG(LogTemp, Error, TEXT("ManusMCP: Failed to create HTTP router on port %d"), Port);
		return false;
	}
	...
	Router->BindRoute(FHttpPath(TEXT("/api/health")), EHttpServerRequestVerbs::VERB_GET,
		FHttpRequestHandler::CreateLambda(
			[this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
			{
				...
				J->SetStringField(TEXT("server"), TEXT("ManusMCP"));
			...
			[this](const FHttpServerRequest& Request, const FHttpResultCallback& OnComplete)
			{
				...
				if (bIsEditor)
				{
					J->SetStringField(TEXT("error"),
						TEXT("Cannot shut down the editor's ManusMCP server. Close the editor instead."));
				}
				else
				{
					J->SetStringField(TEXT("status"), TEXT("shutting_down"));
					RequestEngineExit(TEXT("ManusMCP /api/shutdown"));
				}
				TUniquePtr<FHttpServerResponse> R = FHttpServerResponse::Create(
					JsonToString(J), TEXT("application/json"));
				OnComplete(MoveTemp(R));
	...
	UE_LOG(LogTemp, Display, TEXT("ManusMCP: HTTP server started on port %d (%s mode)"),
		Port, bIsEditor ? TEXT("editor") : TEXT("commandlet"));

	return true;
}

void FManusMCPServer::Stop()
{
	if (bRunning)
	{
		FHttpServerModule& HttpModule = FModuleManager::LoadModuleChecked<FHttpServerModule>("HTTPServer");
		...
		bRunning = false;
		UE_LOG(LogTemp, Display, TEXT("ManusMCP: HTTP server stopped."));
	}
}

bool FManusMCPServer::ProcessOneRequest()
{
