// ManusMCPServer.h
// Core HTTP server for ManusMCP. Plain C++ class (not a UCLASS) that owns all
...
struct FManusPinConnectionRecord
{
...
struct FManusGraphSnapshotData
{
	TArray<FManusNodeRecord> Nodes;
	TArray<FManusPinConnectionRecord> Connections;
};

struct FManusGraphSnapshot
{
	FString SnapshotId;
	FString BlueprintName;
	FString BlueprintPath;
	FDateTime CreatedAt;
	TMap<FString, FManusGraphSnapshotData> Graphs; // graphName -> data
};
...
private:
	...
	// ---- Snapshot storage ----
	TMap<FString, FManusGraphSnapshot> Snapshots;
	static const int32 MaxSnapshots = 50;
	FString GenerateSnapshotId(const FString& BlueprintName);
	FManusGraphSnapshotData CaptureGraphSnapshot(UEdGraph* Graph);
